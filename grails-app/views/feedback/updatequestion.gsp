...
<script type="text/javascript">
  var sErrorMsg = '';
  ['file','atext','qtext'].forEach(function(ids){
  ...
  if(${errorcode.size()?1:0}){
    errorcode.forEach(function(err){
      switch (err) {
        ...
        case 2: sErrorMsg+='<li>${message(code:"error.blank.message",args:["Текст ответа"])}</li>'; window.top.document.getElementById('atext').addClassName('red'); break;
        case 3: sErrorMsg+='<li>${message(code:"error.blank.message",args:["Вопрос"])}</li>'; window.top.document.getElementById('qtext').addClassName('red'); break;
