<g:formRemote id="payrequestAddForm" name="payrequestAddForm" url="[action:'addsrpayrequest']" method="post" onSuccess="processaddpayrequestResponse(e)" style="display:none">
      ...
      <g:each in="${payrequests}" var="record">
        <tr align="center">
          ...
          <g:if test="${iscanedit&&record.modstatus==0}">
            &nbsp;&nbsp;<g:remoteLink class="button" style="z-index:1" title="Удалить" url="${[controller:controllerName, action:'deletesrpayrequest', id:service.id, params:[payrequest_id:record.id]]}" before="if(!confirm('Вы действительно хотите удалить заявку?')) return false" onSuccess="getPayrequests()"><i class="icon-trash"></i></g:remoteLink>
          </g:if>
          </td>
        </tr>
      </g:each>
...
<g:formRemote name="payrequestForm" url="[action:'srpayrequest', params:[agr_id:service.id]]" update="payrequest" onComplete="\$('errorpayrequestlist').up('div').hide();jQuery('#payrequestAddForm').slideDown();" style="display:none">
