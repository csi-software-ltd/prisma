<g:formRemote id="payrequestAddForm" name="payrequestAddForm" url="[action:'addlzpayrequest']" method="post" onSuccess="processaddpayrequestResponse(e)" style="display:none">
      ...
      <g:each in="${payrequests}" var="record">
        <tr align="center">
          ...
          <g:if test="${record.modstatus==0}">
            &nbsp;&nbsp;<g:remoteLink class="button" style="z-index:1" url="${[controller:controllerName,action:'deletelzpayrequest', id:record.id, params:[agr_id:lizing.id]]}" title="Удалить" onSuccess="getPayrequests()"><i class="icon-trash"></i></g:remoteLink>
          </g:if>
          </td>
        </tr>
      </g:each>
...
<g:formRemote name="payrequestForm" url="[action:'lzpayrequest', params:[agr_id:lizing.id]]" update="payrequest" onComplete="\$('errorpayrequestlist').up('div').hide();jQuery('#payrequestAddForm').slideDown();" style="display:none">
