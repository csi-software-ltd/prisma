<html>
  <head>
    <title>Prisma: Заявки на платежи по лицензиям</title>
    ...
    <g:javascript>
      ...
      function togglecheck(){
        if(document.getElementById('groupcheckbox').checked)
          jQuery('#createLicensePayrequestsForm :checkbox:not(:checked)').each(function(){ this.checked=true; });
        else
          jQuery('#createLicensePayrequestsForm :checkbox:checked').each(function(){ this.checked=false; });
      }
    </g:javascript>
  </head>
  <body onload="\$('form_submit_button').click();">
    <h3 class="fleft">Заявки на платежи по лицензиям</h3>
    ...
    <div id="filter" class="padtop filter">
      <g:formRemote name="requestListForm" url="${[action:'licensepayrequestslist']}" method="post" update="list">
