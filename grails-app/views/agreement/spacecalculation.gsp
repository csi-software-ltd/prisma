...
<label class="auto" for="spacecalc_is_dop" disabled>
  <input type="checkbox" id="spacecalc_is_dop" name="is_dop" value="1" <g:if test="${spacecalc.is_dop}">checked</g:if> disabled />
  За доп. услуги
</label>
...
<g:if test="${iscanedit}">
  ...
  <input type="reset" class="button" value="Отмена" onclick="jQuery('#spacecalcAddForm').slideUp();"/>
</div>
