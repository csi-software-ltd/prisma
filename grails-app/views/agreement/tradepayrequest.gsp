...
<input type="text" class="auto" id="payrequest_summa" name="summa" value="${number(value:payrequest?.summa?:agr.summa)}"/>
<label for="payrequest_paydate">Дата платежа:</label>
<g:datepicker class="normal nopad" name="payrequest_paydate" value="${String.format('%td.%<tm.%<tY',payrequest?.paydate?:new Date())}"/><br/>
...
<label for="payrequest_tobank">Банк получателя:</label>
<g:select class="fullline" id="payrequest_tobank" name="tobank" value="${payrequest?.tobankbik?:tobanks.size()==1?tobanks[0].id:''}" from="${tobanks}" optionKey="id" optionValue="name" noSelection="${['':'не выбран']}"/>
<label for="payrequest_destination">Назначение:</label>
<label class="auto" for="payrequest_is_nds">
  <input type="checkbox" id="payrequest_is_nds" name="is_nds" value="1" <g:if test="${payrequest?.is_nds}">checked</g:if> />
  Наличие НДС
</label>
<g:textArea id="payrequest_destination" name="destination" value="${payrequest?.destination?:agr.description}" />
<g:if test="${iscantag}">
