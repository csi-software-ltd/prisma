...
<input type="text" class="auto" id="payrequest_summa" name="summa" value="${number(value:payrequest?.summa?:defaultdata?.summa)}"/>
...
<label class="auto" for="payrequest_is_nds">
  <input type="checkbox" id="payrequest_is_nds" name="is_nds" value="1" <g:if test="${payrequest?.is_nds?:ndsdef}">checked</g:if> />
  Наличие НДС
</label>
