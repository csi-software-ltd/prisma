...
<g:select class="fullline" id="payrequest_tobank" name="tobank" value="${payrequest?.tobankbik?:agr.ebank_id}" from="${tobanks}" optionKey="id" optionValue="name" noSelection="${['':'не выбран']}"/>
<label for="payrequest_destination">Назначение:</label>
<label class="auto" for="payrequest_is_nds">
  <input type="checkbox" id="payrequest_is_nds" name="is_nds" value="1" <g:if test="${payrequest?.is_nds||agr.is_nds}">checked</g:if> />
  С НДС
</label>
