...
<g:if test="${searchresult.records}">
  <div id="resultList">
    <g:formRemote name="createLicensePayrequestsForm" url="[action:'createlcpayrequests']" onSuccess="\$('form_submit_button').click();">
    <table class="list" width="100%" cellpadding="0" cellspacing="0" border="0">
      ...
      <g:if test="${record.summa>0}">
        ...
        <g:if test="${!inrequest.modstatus}">
          ...
          <td><g:link controller="agreement" action="license" id="${record.license_id}">${record.company_name}</g:link></td>
          <td>${record.sro_name}</td>
          <td>${intnumber(value:record.summa)}</td>
          <td>Оплата лицензии по договору ${record.anumber} от ${String.format('%td.%<tm.%<tY',record.adate)}</td>
