  ...
  if(${errorcode.size()?1:0}){
    errorcode.forEach(function(err){
      switch (err) {
        ...
        case 2: sErrorMsg+='<li>${message(code:"error.blank.message",args:["Скан"])}</li>'; window.top.document.getElementById('file').addClassName('red'); break;
        case 100: sErrorMsg+='<li>${message(code:"error.bderror.message")}</li>'; break;
      }
    });
    window.top.document.getElementById("errorissuancescanlist").innerHTML=sErrorMsg;
    window.top.document.getElementById("errorissuancescanlist").up('div').show();
  } else
    window.top.closeIssuance();
</script>