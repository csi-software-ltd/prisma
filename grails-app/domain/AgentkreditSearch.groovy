...
/////////agentperiod////////////////////
  ...
  def csiSelectKredits(iAgentagrId){
    ...
    hsSql.select="*, company.name as client_name, kredit.agentsum as kr_summa, agentperiod.startdate as period_start, agentperiod.enddate as period_end, (select count(*) from agentkreditplan where agentkreditplan.agentkredit_id=agentkredit.id) as periodexist, (select count(*) from agentrate where agentrate.agentkredit_id=agentkredit.id) as agentexist, (select sum(rate) from agentrate where agentrate.agentkredit_id=agentkredit.id and is_sub=0) as agentpercent, (select sum(debt) from agentkreditplan where agentkreditplan.agentkredit_id=agentkredit.id and agentkreditplan.modstatus<2 and is_last=1) as lastbodydebt, bank.shortname as bank_name"
    hsSql.from='agentkredit, kredit, company, bank, agentperiod'
    hsSql.where="agentkredit.kredit_id=kredit.id and kredit.client=company.id and kredit.bank_id=bank.id and agentperiod.id=agentkredit.agentperiod_id"+
