class Valutarate { 
  ...
  def csiSearchToday(iValutaId){
    ...
    hsSql.select='*'
    hsSql.from='valutarate'
    hsSql.where='vdate=CURDATE() and valuta_id in (840,978)'+((iValutaId>0)?' AND valuta_id=:valuta_id':'')
    if(iValutaId>0){
      hsLong['valuta_id']=iValutaId
      return searchService.fetchData(hsSql,hsLong,null,null,null,Valutarate.class,-1,true)[0]
    } else
  ...
  def csiSearchTomorrow(iValutaId){
    ...
    hsSql.select='*'
    hsSql.from='valutarate'
    hsSql.where='vdate=CURDATE()+1 and valuta_id in (840,978)'+((iValutaId>0)?' AND valuta_id=:valuta_id':'')    
    if(iValutaId>0){
      hsLong['valuta_id']=iValutaId
      return searchService.fetchData(hsSql,hsLong,null,null,null,Valutarate.class,-1,true)[0]
    } else
